...
class DockerSwarm {
  ...
  public static function loadById($id) {
      ...
      $cm->name             = $row['DSname'];
      $cm->admin            = $row['DSadmin'];
      $cm->backupAdmin      = $row['DSbackupAdmin'];
...
SELECT Xname AS name FROM Runs WHERE DSname='%s';", $id, $id);
  ...
  public static function loaddep($id) {
      ...
      $db = Db::instance();
      // Database query
      $q = sprintf("SELECT F5name AS name FROM Routes WHERE Aname IN (SELECT Aname AS name FROM Hosts WHERE DSname='%s')
UNION
SELECT Aname AS name FROM Hosts WHERE DSname='%s';", $id, $id);
      // Do the query
      $result = $db->query($q);
      // If nothing found
      if($result->num_rows == 0) {
  ...
  public static function updateAdmin($name, $admin) {
      $db = Db::instance();
      // Database query
      $q = sprintf("UPDATE `%s` SET admin = '%s' WHERE DSname = '%s';", self::DB_TABLE, $admin, $name);
      echo($q);
      // Do the query
      $result = $db->query($q);
      //Return result
